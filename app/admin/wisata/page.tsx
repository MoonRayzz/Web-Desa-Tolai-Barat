...
import { getAllWisata, createWisata, updateWisata, deleteWisata } from "@/lib/firebase/wisata";
import type { Wisata, WisataKategori } from "@/types";
...
export default function AdminWisataPage() {
  ...
  function openEdit(w: Wisata){ setForm({ name: w.name, description: w.description, image: w.image, kategori: w.kategori, featured: w.featured }); setEditId(w.id); }
        ...
        <div style={{
          ...
          <form onSubmit={handleSubmit}>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: "10px", marginBottom: "20px" }}>
              ...
              <label htmlFor="featured" style={{ fontSize: "0.875rem", color: "var(--color-ocean-700)" }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(280px, 1fr))", gap: "16px" }}>
        {loading ? (
          ...
          <div key={w.id} style={{
            ...
            boxShadow: "var(--shadow-card)", border: "1px solid var(--color-ocean-100)",
          }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: "8px" }}>
              <div style={{ flex: 1 }}>
                ...
                <div style={{ display: "flex", gap: "6px", flexWrap: "wrap", marginBottom: "8px" }}>
                  <span style={{ background: "var(--color-ocean-100)", color: "var(--color-ocean-700)", fontSize: "0.68rem", fontWeight: 600, padding: "2px 8px", borderRadius: "9999px" }}>
                    {w.kategori}
                  </span>
                  {w.featured && (
                    <span style={{ background: "var(--color-gold-100)", color: "var(--color-gold-700)", fontSize: "0.68rem", fontWeight: 600, padding: "2px 8px", borderRadius: "9999px" }}>
                      ⭐ Unggulan
                    </span>
                  )}
                </div>
                <p style={{ fontSize: "0.8rem", color: "var(--color-ocean-600)", lineHeight: 1.6,
                  display: "-webkit-box", WebkitLineClamp: 2, WebkitBoxOrient: "vertical" as const, overflow: "hidden" }}>
                  {w.description}
                </p>
              </div>
            </div>
            <div style={{ display: "flex", gap: "8px", marginTop: "14px", paddingTop: "12px", borderTop: "1px solid var(--color-ocean-100)" }}>
              ...
              <button onClick={() => handleDelete(w.id, w.name)} style={{
                flex: 1, padding: "7px", borderRadius: "8px",
